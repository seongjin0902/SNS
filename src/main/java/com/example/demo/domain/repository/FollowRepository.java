...
//    @Query("SELECT f.following.email FROM Follow f WHERE f.follower.email = :followerEmail")
    ...
    @Query("SELECT u FROM Follow f INNER JOIN User u ON f.follower.email = u.email WHERE f.following.email = :followerEmail")
    List<User> findByFollowerNickname(@Param("followerEmail") String followerEmail);
    ...
    //나를 팔로우 누른 사람의 수
    @Query("SELECT COUNT(f.following.email) as cnt FROM Follow f WHERE f.following.email=:followerEmail GROUP BY f.following.email")
